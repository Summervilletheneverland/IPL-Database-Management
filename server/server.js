...
app.post("/register",async(req,res)=>{
    ...
    try{
                ...
                console.log(result.rowCount)
                  if(result.rowCount==0){
                    const insert_accounts = `INSERT INTO ACCOUNTS VALUES('${email}','${username}','${password}');`
                    try{
...
app.post('/get_seasons',async(req,res)=>{
    const result=await pool.query(`SELECT SEASON_ID,YEAR FROM SEASON;`)
    res.json(result.rows);
    // console.log(result.rows)
})
app.post('/get_categories',async(req,res)=>{
    const v_performance='v_performance'
    const result=await pool.query(`SELECT COLUMN_NAME
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_NAME = '${v_performance}' AND COLUMN_NAME NOT IN('SEASON_ID','PLAYER_NAME');`)
    res.json(result.rows);
    //console.log(result.rows)
})
app.post('/get_stats',async(req,res)=>{
    ...
    const result=await pool.query(`SELECT * FROM v_performance where season_id='${season}' order by ${category} desc;`)
    res.json(result.rows);
   // console.log(result.rows)
})
app.post('/retain',async(req,res)=>{
    const {pid,sid}=req.body;
    console.log(pid,sid);
    const check_retention=`SELECT CHECK_RETENTION(${pid},'${sid}')`
    const result=await pool.query(check_retention)
    console.log(result.rows);
})
...
app.post("/admin_search",async(req,res)=>{
    const s=req.body;
    console.log(s.name)
    const search_player=`SELECT * FROM PLAYERS natural JOIN AUCTION WHERE PLAYER_NAME ILIKE '%${s.name}%' and season_id=(select(max(season_id)) from season);`
